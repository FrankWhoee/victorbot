...
async def main(message: discord.Message, client: discord.Client, data: dict, command: dict) -> bool:
    ...
    embed.add_field(name="Drawings", value=str(round(pred["drawings"] * 100)) + "%", inline=True)
    embed.add_field(name="Hentai", value=str(round(pred["hentai"] * 100)) + "%", inline=True)
    embed.add_field(name="Neutral", value=str(round(pred["neutral"] * 100)) + "%", inline=True)
    embed.add_field(name="Porn", value=str(round(pred["porn"] * 100)) + "%", inline=True)
    embed.add_field(name="Sexy", value=str(round(pred["sexy"] * 100)) + "%", inline=True)
