...
async def main(message: discord.Message, command: dict, victor: Victor) -> bool:
    ...
    for target_user in target_users:
        if target_user.voice is None:
            if "grants" in victor.guild_data(message.guild.id):
                victor.guild_data(message.guild.id)["grants"][str(target_user.id)] = channel.id
            else:
                victor.guild_data(message.guild.id)["grants"] = {str(target_user.id): channel.id}
            ...
            embed = discord.Embed(title="Grant",
                                  description=f"{target_user.mention} has been moved to {channel.mention}.",
                                  color=0x00ff00)
            await message.channel.send(embed=embed)
